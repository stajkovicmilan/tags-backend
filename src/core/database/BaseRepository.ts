...
export class BaseRepository<T> {
    ...
    public async findAndSort(query: any, sortQuery?: any, skipLimit?: any): Promise<T[]> {
        ...
        let queryBuild = await DB.db.collection(this.entityName).find(query);
        if (sortQuery) {
            queryBuild = queryBuild.sort(sortQuery);
        }
        if (skipLimit) {
            queryBuild = queryBuild.skip(Number(skipLimit.skip)).limit(Number(skipLimit.limit));
        }
    ...
    public async create(entity: T): Promise<T> {
        this.tryToApplyTimestampAndStatus(entity);
    ...
    public async deleteMany(entities: T[]): Promise<boolean> {
        for (let i = 0; i < entities.length; i++) {
    ...
    protected normalizeSearchQuery(query: any): any {
        ...
        if (!!query.id) {
            if (query.id.$in && Array.isArray(query.id.$in)) {
                query._id = {
                    $in: query.id.$in.map((x) => {
                        return this.deserializeObjectId(x);
                    }),
                };
                delete query.id;
            } else {
    ...
    private tryToApplyTimestampAndStatus(entity: any) {
        try {
            if (!entity.createdAt) {
